...
def write_dshield_format_of_propagator_results(
    propagator_results: list[dict], epoch_dict: dict, out_dir: str
) -> None:
    """
    ...
    for sc in propagator_results:
        ...
        if len(ephemeris_seconds) >= 2:
            ...
            mission_days = (
                ephemeris_seconds[-1] - ephemeris_seconds[0]
            ) / 86400.0
        else:
            step_seconds = 0.0
            mission_days = 0.0
        ...
        with open(out_fp, "w", newline="", encoding="utf-8") as f:
            ...
            # CSV header
            f.write(
                "time index,x [km],y [km],z [km],vx [km/s],vy [km/s],vz [km/s]\n"
            )
...
def write_dshield_format_of_contact_results(
    ...
    def _to_dt(t):
        if isinstance(t, dict):
            if (
                t.get("time_format") != "GREGORIAN_DATE"
        ...
        for g_idx, gs in enumerate(contacts):
            ...
            with open(out_fp, "w", newline="", encoding="utf-8") as f:
                ...
                for interval in contact_info:
                    ...
                    start_idx = int(
                        round(
                            (start_dt - epoch_dt).total_seconds()
                            / step_size_seconds
                        )
                    )
                    end_idx = int(
                        round(
                            (end_dt - epoch_dt).total_seconds()
                            / step_size_seconds
                        )
                    )
...
def write_dshield_format_of_eclipse_results(
    eclipse_results: list[dict],
    out_dir: str,
    epoch_dict: dict,
    step_size_seconds: float,
) -> None:
    ...
    for sc in eclipse_results:
        ...
        with open(out_fp, "w", newline="", encoding="utf-8") as f:
            ...
            for interval in eclipse_info:
                ...
                end_idx = int(
                    round(
                        (end_dt - epoch_dt).total_seconds() / step_size_seconds
                    )
                )
                f.write(f"{start_idx},{end_idx}\n")
...
def write_dshield_format_of_point_coverage_results(
    ...
    for sc in coverage_results:
        ...
        for sensor_idx, sensor in enumerate(sensors):
            ...
            if not coverage_info:
                print(
                    f"Skipping {sc_name} sensor {sensor_name} id: {sensor_id}: no coverage data"
                )
                continue
            ...
            with open(out_fp, "w", newline="", encoding="utf-8") as f:
                ...
                for idx in range(len(ephemeris_seconds)):
                    ...
                    time_idx = int(
                        round(
                            (ephemeris_seconds[idx] - epoch_ephemeris_seconds)
                            / step_size_seconds
                        )
                    )
...
def write_dshield_format_of_gnssr_coverage_results(
    coverage_results: list[dict],
    out_dir: str,
    epoch_dict: dict,
    epoch_ephemeris_seconds: float,
...
def aggregate_gnssr_coverage_by_time(
    coverage_file: str, output_file: str
) -> None:
    """
...
def write_dshield_format_of_specular_trajectory_results(
    ...
    for sc in specular_trajectories:  # iterate over all receiver spacecrafts
        ...
        all_spacecraft_specular_info = sc.get(
