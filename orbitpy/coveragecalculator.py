...
class CoverageFactory:
    ...
    def from_dict(cls, specs: Dict[str, Any]) -> object:
        """Retrieves an instance of the appropriate coverage calculator based on specifications.
        ...
        Raises:
            KeyError: If the "coverage_type" key is missing in the specifications dictionary.
        ...
        if coverage_type_str is None:
            raise KeyError(
                'Coverage calculator type key "coverage_type" not found in specifications dictionary.'
            )
...
class PointCoverage:
    """A coverage calculator class which handles coverage calculation for an array of target points."""
    ...
    def calculate_coverage(self, 
        ...
        Args:
            ...
            frame_graph (FrameGraph): The frame graph containing the necessary transformations
                between frames.
            times (AbsoluteDateArray): An array of time points for which the coverage is calculated at discrete instances.
            buff_size (int): The size of the buffer for parallel computation. Should be at least the number of cores.
        Returns:
        ...
        rot_fov_to_target, w = frame_graph.get_orientation_transform(fov_frame, target_frame, times)
        ...
        if isinstance(fov, CircularFieldOfView):
            ...
            up_TARGET_source = kcl.TransformedVector3dSource(up_FOV_source,FOV_to_TARGET_source,buff_size)
            right_TARGET_source = kcl.TransformedVector3dSource(right_FOV_source,FOV_to_TARGET_source,buff_size)
            ...
            fov_source = kcl.VectorAngleRectViewSource(pos_fov_TARGET_source,
                up_TARGET_source, right_TARGET_source, up_angle_source, right_angle_source, buff_size)
        ...
        cov_source = kcl.CovSource(grid_source,None,total_view,buff_size_coverage)
        ...
        print("Starting coverage calculation...") # TODO: Remove this line
        kcl.driveCoverage(buff_size,start_time,stop_time,driver,[])
        print("Finished coverage calculation...") # TODO: Remove this line
