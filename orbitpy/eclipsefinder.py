...
class EclipseFinder:
    ...
    @staticmethod
    def check_line_of_sight(object1_pos: np.ndarray, object2_pos: np.ndarray, obstacle_radius: float) -> bool:
        """
        Determine if line-of-sight exists between two objects with a spherical obstacle in between.

        This method uses the algorithm described on Page 198 of "Fundamentals of Astrodynamics and Applications" 
        by David A. Vallado (first algorithm).
        ...
        Note:
            The frame of reference for describing the object positions must be centered at the spherical obstacle.
        """
    ...
    def _in_eclipse_single_item(et: float, e2o: np.ndarray) -> bool:
        ...
        # Get Sun position relative to Earth (in the J2000 frame ~ ICRF frame in SPICE) at the given time
        ...
        return (not EclipseFinder.check_line_of_sight(earth_to_sun, e2o, EARTH_POLAR_RADIUS))

    @staticmethod
    def _in_eclipse_array(et_array: np.ndarray, earth_to_obj_array: np.ndarray) -> np.ndarray:
        """
    ...
    def execute(self, frame_graph: FrameGraph,
                ...
                position: Optional[Union[GeographicPosition, Cartesian3DPosition]] = None,
                state: Optional[Union[StateSeries, PositionSeries, CartesianState]] = None) -> Union[bool, list[bool]]:
        """Run the eclipse detection algorithm with Earth (spherical model with the radius as the Polar radius) 
            as the occluding body.
        ...
        if (time is not None and position is not None) and state is not None or \
            (time is not None and position is None) and state is not None or \
            (time is None and position is not None) and state is not None:
