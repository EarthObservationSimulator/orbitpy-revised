...
Maximum deviation in contact interval start and stop times were found to be
within 14 seconds for the NOAA-20 satellite, and 32.5 seconds for the CYGFM-01 satellite.
...
However, in the GMAT simulations, the maximum step-size in the trajectory
(state) computation was 10 seconds and in the contact interval computation,
the step size was 10 seconds. Errors greater than 20 seconds are puzzling.
"""
...
class TestElevationAwareContactFinder(unittest.TestCase):
    """Validation tests for the ElevationAwareContactFinder using GMAT-generated data.
    """
    def setUp(self):
        ...
        if "noaa_trajectory.pkl" in os.listdir(self.noaa_dir):
            ...
            with open(
                os.path.join(self.noaa_dir, "noaa_trajectory.pkl"), "rb"
            ) as f:
                self.noaa_trajectory = pickle.load(f)
        else:
            self.noaa_trajectory = parse_gmat_state_file(
                os.path.join(self.noaa_dir, "Cartesian.txt")
            )
            with open(
                os.path.join(self.noaa_dir, "noaa_trajectory.pkl"), "wb"
            ) as f:
                pickle.dump(
                    self.noaa_trajectory, f, protocol=pickle.HIGHEST_PROTOCOL
                )
        ...
        if "cygfm01_trajectory.pkl" in os.listdir(self.cygfm01_dir):
            ...
            with open(
                os.path.join(self.cygfm01_dir, "cygfm01_trajectory.pkl"), "rb"
            ) as f:
                self.cygfm01_trajectory = pickle.load(f)
        else:
            self.cygfm01_trajectory = parse_gmat_state_file(
                os.path.join(self.cygfm01_dir, "Cartesian.txt")
            )
            with open(
                os.path.join(self.cygfm01_dir, "cygfm01_trajectory.pkl"), "wb"
            ) as f:
                pickle.dump(
                    self.cygfm01_trajectory, f, protocol=pickle.HIGHEST_PROTOCOL
                )
    ...
    def test_elevation_contact_finder_with_station_1_noaa20(self):
        ...
        gmat_contacts = parse_gmat_contact_file(
            os.path.join(self.noaa_dir, "ContactLocator1.txt")
        )
        ...
        self.assertEqual(
            len(gmat_contacts), len(result.contact_intervals())
        )  # match number of contact intervals

        for gmat_contact_item, orbitpy_contact_item in zip(
            gmat_contacts, result.contact_intervals()
        ):
    ...
    def test_elevation_contact_finder_with_other_stations_noaa20(self):
        ...
        for station in [
            ...
            with self.subTest(station=station.name):
                gmat_contacts = parse_gmat_contact_file(
                    os.path.join(
                        self.noaa_dir, f"ContactLocator{station.name[-1]}.txt"
                    )
                )
                ...
                for i, (gmat_contact_item, orbitpy_contact_item) in enumerate(
                    zip(gmat_contacts, result.contact_intervals())
                ):
                    self.assertAlmostEqual(
                        gmat_contact_item[0].to_spice_ephemeris_time(),
                        orbitpy_contact_item[0].to_spice_ephemeris_time(),
    ...
    def test_elevation_contact_finder_with_station_1_cygfm01(self):
        """Compare GMAT ContactLocator results with ElevationAwareContactFinder
        results for CYGFM-01. Errors are expected since the ElevationAwareContactFinder
        calculated the contacts at the trajectory time-steps only and evaluates
        the contact interval, whereas GMAT possibly
        uses a continuous technique to find more accurate contact times.
        """
        gmat_contacts = parse_gmat_contact_file(
            os.path.join(self.cygfm01_dir, "ContactLocator1.txt")
        )
    ...
    def test_elevation_contact_finder_with_other_stations_cygfm01(self):
        """Compare GMAT ContactLocator results with ElevationAwareContactFinder
        results for CYGFM-01. Errors are expected since the ElevationAwareContactFinder
        calculated the contacts at the trajectory time-steps only and 
        evaluates the contact interval, whereas GMAT possibly
        uses a continuous technique to find more accurate contact times.
        """
        ...
        for station in [
            ...
            with self.subTest(station=station.name):
                gmat_contacts = parse_gmat_contact_file(
                    os.path.join(
                        self.cygfm01_dir,
                        f"ContactLocator{station.name[-1]}.txt",
                    )
                )
