...
from eosimutils.state import Cartesian3DPosition, GeographicPosition, CartesianState
...
class TestEclipseFinder(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.registry = FrameGraph() # ICRF_EC and ITRF frames are automatically registered.
    ...
    def test_eclipse_geographic_position(self):
        ...
        expected_result = False
        self.assertEqual(result, expected_result, msg=f"Latitude: {random_latitude} and Elevation: {random_elevation} was used.")
    ...
    def test_eclipse_single_position_multiple_time(self):
        ...
        self.assertTrue(np.array_equal(result, expected_results), msg=f"Latitude: {random_latitude}, Longitude: {random_longitude} was used.")
    ...
    def test_eclipse_with_cartesian_state(self):
        ...
        random_date = f"2025-{random.randint(1, 12):02d}-{random.randint(1, 28):02d}T{random.randint(0, 23):02d}:{random.randint(0, 59):02d}:{random.randint(0, 59):02d}.000"
        ...
        expected_result = False
        self.assertEqual(result, expected_result, msg=f"Date: {random_date} and Position: {random_position} was used.")
    ...
    def test_eclipse_with_state_series(self):
        ...
        state_series = StateSeries.from_dict({
            ...
            "data": [
                [[7000.0, 250.0, 0.0], [7000.0, 250.0, 0.0]],  # Same Position, close to the 0 deg longitude.
    ...
    def test_object_inside_earth(self):
        ...
        result = self.eclipse_finder.execute(
            frame_graph=self.registry,
            time=time,
            position=inside_earth_position
        )
