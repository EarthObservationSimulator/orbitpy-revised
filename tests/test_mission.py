...
def uniform_lat_lon_spacing_grid(lat_lower_bound: float, lat_upper_bound: float, lat_step_deg: float, 
    ...
    """Return uniformly spaced points on a latitude/longitude grid given the bounds."""
    ...
    return Cartesian3DPositionArray.from_geographic_positions([
        GeographicPosition(latitude_degrees=lat, longitude_degrees=lon, elevation_m=0.0)
...
class TestMission_2(unittest.TestCase):
    ...
    - The first spacecraft is in a polar orbit and has two sensors with non-overlapping fields of view. 
      Thus, the covered points at a given time should not overlap.
    
    - The second spacecraft is in a low-inclination orbit and has one sensor. The covered points should be roughly
    ...
    def setUp(self):
        ...
        points_array = uniform_lat_lon_spacing_grid(-90, 90, 1, -180, 180, 1)
    ...
    def test_execute_eclipse_finder(self):
        ...
        for idx, entry in enumerate(eclipse_results):
            self.assertIsInstance(entry, dict)
            self.assertIsInstance(entry.get("spacecraft_id"), str)
            self.assertIn(entry.get("spacecraft_id"), m.spacecrafts[idx].identifier)
            self.assertIn("eclipse_info", entry)
    ...
    def test_execute_gs_contact_finder(self):
        ...
        for spc_idx, entry in enumerate(contact_info):
            self.assertIsInstance(entry, dict)
            self.assertIsInstance(entry.get("spacecraft_id"), str)
            self.assertIn(entry.get("spacecraft_id"), m.spacecrafts[spc_idx].identifier)
            self.assertIn("contacts", entry)
            self.assertIsInstance(entry["contacts"], list)
            self.assertEqual(len(entry["contacts"]), len(m.ground_stations))  # match the number of ground-stations
            ...
            for gs_idx, gs_entry in enumerate(entry["contacts"]):
                ...
                self.assertIn(gs_entry.get("ground_station_id"), m.ground_stations[gs_idx].identifier)
                self.assertIn("contact_info", gs_entry)
                self.assertIsInstance(gs_entry["contact_info"], ContactInfo)
    ...
    def test_polar_spacecraft_sensor_coverages_do_not_overlap(self):
        mission = Mission.from_dict(self.mission_dict)
        propagated = mission.execute_propagation()
        coverage = mission.execute_coverage_calculator(propagated)
        polar_entry = next(item for item in coverage if item["spacecraft_id"] == "cf4cea95-e935-4314-a235-02ea87040b11")
        ...
        sensor1_indices = set(sensor1_cov.get_covered_point_indices())
        sensor2_indices = set(sensor2_cov.get_covered_point_indices())
        self.assertEqual(sensor1_indices & sensor2_indices, set())

    def test_low_inclination_spacecraft_coverage_within_latitude_bounds(self):
        mission = Mission.from_dict(self.mission_dict)
        propagated = mission.execute_propagation()
        coverage = mission.execute_coverage_calculator(propagated)
        iss_entry = next(item for item in coverage if item["spacecraft_id"] == "e37db1f2-4dc3-4655-8ffc-50d471c349b9")
        ...
        point_indices = iss_cov.get_covered_point_indices()
        covered_points = self.points[point_indices]
        latitudes = np.degrees(np.arcsin(np.clip(covered_points[:, 2] / np.linalg.norm(covered_points, axis=1), -1.0, 1.0)))
        self.assertTrue(np.max(np.abs(latitudes)) < 60.0)
    '''
