...
class TestPropagatorFactory(unittest.TestCase):
    ...
    def test___init__(self):
        ...
        self.assertIn("SGP4 PROPAGATOR", factory._creators) # pylint: disable=protected-access
        self.assertEqual(factory._creators["SGP4 PROPAGATOR"], SGP4Propagator) # pylint: disable=protected-access

    def test_register_propagator(self):
        """Test registering a new propagator in the factory."""
        factory = PropagatorFactory()
        factory.register_propagator(
            "New Propagator", TestPropagatorFactory.DummyNewPropagator
        )

        # Verify the new propagator is registered
        self.assertIn("New Propagator", factory._creators) # pylint: disable=protected-access
        self.assertEqual(
            factory._creators["New Propagator"], # pylint: disable=protected-access
    ...
    def test_get_propagator_invalid_type(self):
        ...
        with self.assertRaises(ValueError):
            factory.get_propagator({"propagator_type": "Invalid Propagator"})
